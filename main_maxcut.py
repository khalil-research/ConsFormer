...
def main():
    ...
    model_name = f"CUT{vertices_count}-{rpe}rpe-{subset_threshold}-{ape_dim}pe-{mixing_strategy}mixer-destroy{destroy}-{loss_function_name}-{'softmax' if greedy_decode else 'gumbel'}{tau}-{optimizer_name}-dropout{dropout}-embed{embedding_size}-{head_count}h{num_layers}l-bs{batch_size}-lr{learning_rate}"
