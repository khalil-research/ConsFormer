...
from consformer.csptask import GraphColoringTask
...
def main():
    ...
    colors_count = args.colors_used
    input_size = colors_count
    output_size = colors_count
    vocab_size = colors_count
    ...
    # dataset
    inputs_path_train = f"data/graph_coloring/graphcoloring_{vertices_count}_vert_{colors_count}_color_train.pt"
    inputs_path_test = f"data/graph_coloring/graphcoloring_{vertices_count}_vert_{colors_count}_color_test.pt"

    acc_func = CustomGCOLAccuracy()  # reports # of all-diff satisfied
    loss_func = loss_functions[loss_function_name]()
    graph_col_task = GraphColoringTask(loss_func, acc_func)
    ...
    if colors_count == 5:
        vertices_test = [50, 100]
    elif colors_count == 10:
        vertices_test = [100, 200]

    run_analysis_graph_coloring(trainer, colors_count, vertices_test)
